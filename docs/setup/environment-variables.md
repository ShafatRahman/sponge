...
| `OPENAI_API_KEY` | No | Required only for Detailed mode. GPT-4o-mini for description enhancement. Task fails fast with a clear error if missing and Detailed mode is requested. |
...
3. **OpenAI**: Get an API key at [platform.openai.com](https://platform.openai.com/api-keys). Only needed for Detailed mode.
